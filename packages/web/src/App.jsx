...
import { Box, Typography, Alert, Button, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Modal, Backdrop, Fade } from '@mui/material'
...
function App() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('buzz-game-sessions', JSON.stringify(finishedSessions))
  }, [finishedSessions])
  ...
  const connectSerialPort = async () => {
    try {
      ...
      if (err.name === 'NotFoundError') {
        ...
        setSerialError('Failed to connect to serial port: ' + err.message)
      }
  ...
  return (
    <Box className={`${styles.container} ${isPulsing ? styles.pulse : ''}`}>
      <Box className={styles.topRow}>
        ...
        <Paper className={`${styles.counterCell} ${isPulsing ? styles.counterPulse : ''}`} elevation={3}>
          <Box className={styles.digitalDisplay}>
            <Typography variant="h2" className={`${styles.digitalText} ${isPulsing ? styles.counterTextPulse : ''}`}>
              ...
              <TableBody>
                {finishedSessions.length === 0 ? (
                  ...
                  finishedSessions.map((session, index) => (
                    <TableRow
                      key={index}
                      onClick={() => session.imageData && setSelectedSession(session)}
                      style={{ cursor: session.imageData ? 'pointer' : 'default' }}
                    >
      ...
      >
        <Fade in={!!selectedSession}>
          <Box className={styles.imageModalContent} onClick={() => setSelectedSession(null)}>
            {selectedSession && selectedSession.imageData && (
              <Box className={styles.imageContainer} onClick={(e) => e.stopPropagation()}>
                <img
